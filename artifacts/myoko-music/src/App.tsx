...
function AdminPage() {
  ...
  if (!authenticated) {
  return <main className="myoko-page noise flex min-h-[100dvh] items-center justify-center bg-foreground px-5 py-10 text-background"><div className="w-full max-w-md border border-background/25 bg-[#171615] p-7 sm:p-10"><div className="flex items-center gap-3"><img src={logoImage} alt="" className="h-10 w-10 rounded-full object-cover" /><span className="font-display text-xl font-bold">MYOKO MUSIC</span></div><p className="mt-12 font-mono-brand text-[10px] uppercase tracking-[.22em] text-secondary">Private team area</p><h1 className="mt-3 font-display text-5xl font-bold leading-[.9] tracking-[-.08em]">Orders<br /><span className="text-primary">only.</span></h1><p className="mt-5 text-sm leading-relaxed text-background/60">This page is for the Myoko team. Customers do not need an account to place an order.</p><form onSubmit={login} className="mt-8"><input type="text" name="username" autoComplete="username" value="myoko-admin" readOnly tabIndex={-1} aria-hidden="true" className="sr-only" /><label><span className="form-label text-background">Admin password</span><input autoFocus autoComplete="current-password" required type="password" value={password} onChange={(event) => setPassword(event.target.value)} className="form-input border-background/35 text-background placeholder:text-background/35" placeholder="Enter the team password" data-testid="input-admin-password" /></label>{error && <p className="mt-4 text-sm text-secondary" role="alert">{error}</p>}<button type="submit" className="focus-ring mt-6 flex w-full items-center justify-center gap-3 bg-primary px-5 py-4 font-mono-brand text-[10px] uppercase tracking-[.15em] text-primary-foreground hover:bg-accent" data-testid="button-admin-login">Open orders <ArrowRight size={15} /></button></form></div></main>;
  }

  return <main className="myoko-page noise min-h-[100dvh] bg-foreground px-5 py-8 text-background sm:px-8 lg:px-12 lg:py-12">
  <div className="mx-auto max-w-[1280px]">
    <header className="flex flex-col justify-between gap-5 border-b border-background/20 pb-7 sm:flex-row sm:items-end">
      <div>
        <p className="font-mono-brand text-[10px] uppercase tracking-[.22em] text-primary">Private team area</p>
        <h1 className="mt-2 font-display text-5xl font-bold tracking-[-.08em]">Myoko <span className="text-primary">orders.</span></h1>
        <p className="mt-3 text-sm text-background/60">Manage preorders, delivery timing, and handover payment.</p>
      </div>
      <div className="flex gap-3">
        <button className="focus-ring border border-background/40 px-4 py-3 font-mono-brand text-[10px] uppercase tracking-[.12em] hover:bg-background hover:text-foreground" onClick={() => void loadOrders()} disabled={loadingOrders}>{loadingOrders ? 'Refreshing…' : 'Refresh'}</button>
        <button className="focus-ring border border-background/40 px-4 py-3 font-mono-brand text-[10px] uppercase tracking-[.12em] hover:bg-background hover:text-foreground" onClick={() => void logout()}>Log out</button>
      </div>
    </header>

    {error && <p className="mt-6 border border-primary bg-primary/10 px-4 py-3 text-sm text-primary" role="alert">{error}</p>}

    <div className="mt-8 grid gap-4 sm:grid-cols-3">
      <div className="border border-background/20 bg-[#171615] p-5">
        <p className="font-mono-brand text-[9px] uppercase tracking-[.15em] text-background/60">Total orders</p>
        <p className="mt-3 font-display text-4xl font-bold">{orders.length}</p>
      </div>
      <div className="border border-background/20 bg-[#171615] p-5">
        <p className="font-mono-brand text-[9px] uppercase tracking-[.15em] text-background/60">To make</p>
        <p className="mt-3 font-display text-4xl font-bold">{orders.filter((order) => !['completed', 'cancelled'].includes(order.orderStatus)).length}</p>
      </div>
      <div className="border border-background/20 bg-[#171615] p-5">
        <p className="font-mono-brand text-[9px] uppercase tracking-[.15em] text-background/60">Unpaid</p>
        <p className="mt-3 font-display text-4xl font-bold">{orders.filter((order) => order.paymentStatus === 'unpaid').length}</p>
      </div>
    </div>

    <div className="mt-8 space-y-5">
      {orders.length === 0 && <div className="border border-background/20 p-10 text-center">
        <p className="font-display text-2xl font-bold">No orders yet.</p>
        <p className="mt-2 text-sm text-background/60">New website orders will appear here.</p>
      </div>}

      {orders.map((order) => <article key={order.id} className="border border-background/20 bg-[#171615] p-5 sm:p-7">
        <div className="flex flex-col justify-between gap-5 border-b border-background/15 pb-5 lg:flex-row lg:items-start">
          <div>
            <div className="flex flex-wrap items-center gap-3">
              <span className="font-mono-brand text-xs text-primary">{order.orderNumber}</span>
              <span className="font-mono-brand text-[9px] uppercase tracking-[.13em] text-background/45">{new Date(order.createdAt).toLocaleString()}</span>
            </div>
            <h2 className="mt-3 font-display text-3xl font-bold tracking-[-.06em]">{order.customerName}</h2>
            <p className="mt-1 text-sm text-background/60">{order.className} · {order.school} · {order.albumName}</p>
          </div>
          <div className="text-left lg:text-right">
            <p className="font-display text-2xl font-bold">{money(order.total)}</p>
            <p className="mt-1 font-mono-brand text-[9px] uppercase tracking-[.12em] text-background/50">{order.keychainQuantity} keychains / {order.paymentMethod.replaceAll('_', ' ')}</p>
          </div>
        </div>

        <div className="grid gap-5 py-5 md:grid-cols-[1fr_1fr_.8fr]">
          <div>
            <p className="font-mono-brand text-[9px] uppercase tracking-[.14em] text-primary">Offers</p>
            <p className="mt-2 text-sm leading-relaxed">{order.offers.map((offer) => `${offer.quantity} × ${offer.name}`).join(' · ')}</p>
          </div>
          <div>
            <p className="font-mono-brand text-[9px] uppercase tracking-[.14em] text-primary">Delivery</p>
            <p className="mt-2 text-sm">{order.deliveryPreference}</p>
            <p className="mt-1 text-sm text-background/60">{order.deliveryNotes || 'No extra delivery note.'}</p>
          </div>
          <div>
            <p className="font-mono-brand text-[9px] uppercase tracking-[.14em] text-primary">Payment</p>
            <select value={order.paymentStatus} onChange={(event) => void updateOrder(order.id, { paymentStatus: event.target.value as AdminOrder['paymentStatus'] })} className="form-input mt-2 border-background/35 bg-transparent text-background" data-testid={`select-payment-${order.id}`}>
        ...
        <div className="flex flex-col gap-4 border-t border-background/15 pt-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="font-mono-brand text-[9px] uppercase tracking-[.14em] text-primary">Order status</p>
            <select value={order.orderStatus} onChange={(event) => void updateOrder(order.id, { orderStatus: event.target.value as AdminOrder['orderStatus'] })} className="form-input mt-2 border-background/35 bg-transparent text-background sm:w-64" data-testid={`select-order-status-${order.id}`}>
              {adminOrderStatuses.map((status) => <option key={status} value={status}>{status.charAt(0).toUpperCase() + status.slice(1)}</option>)}
            </select>
          </div>
          <p className="max-w-sm text-xs leading-relaxed text-background/50">The customer selected <strong className="text-background/80">{order.deliveryPreference}</strong>. Missing that handover means the order cannot be rescheduled.</p>
        </div>
      </article>)}
    </div>
  </div>
